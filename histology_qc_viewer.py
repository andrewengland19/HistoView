...
======================
Napari-based keyboard-driven QC viewer for multichannel fluorescence
histology datasets.  Supports multi-rat cohorts, per-channel cell counting
with right-click markers, QC annotation flags, and a section manifest for
targeted quantification.

Quick start
-----------
  # First run — writes a default config.toml next to this script
  python histology_qc_viewer.py --init-config

  # Edit config.toml to point at your data, then launch
  python histology_qc_viewer.py

  # Override base directory at runtime (no config edit needed)
  python histology_qc_viewer.py --base ~/Microscopy/MyCohort

  # Load only sections marked include=1 in section_manifest.csv
  python histology_qc_viewer.py --manifest-only

  # Full help
  python histology_qc_viewer.py --help

Expected directory layout
-------------------------
<base_dir>/
    <png_subdir>/               # configurable; default: PNG_fullsize
        <ratID>/
            CH1/  CH2/  CH3/  CH4/
                <stem>_CH1.png  ...
    features/
    ...
    qc_annotations.csv          (written by this tool)
    cell_counts.csv             (written by this tool)
    section_manifest.csv        (written by generate_manifest.py)

Configuration
-------------
All paths and channel labels are set in config.toml (generated by
--init-config).  CLI flags override config values at runtime.
...
Cell counting
-------------
  Right-click   place a marker on the active channel; recorded in
                cell_counts.csv with columns:
...
try:
    ...
    try:
        ...
        tomllib = None                # config.toml loading will be skipped
...
# These are overridden by config.toml and then by CLI flags.
# Run  python histology_qc_viewer.py --init-config  to write a starter
# config.toml next to this script.
# ---------------------------------------------------------------------------

BASE_DIR        = Path.home() / "Microscopy" / "MyCohort"
PNG_SUBDIR      = "PNG_fullsize"          # relative to BASE_DIR
PNG_DIR         = BASE_DIR / PNG_SUBDIR
...
# Path to config.toml (searched next to this script, then cwd)
_SCRIPT_DIR     = Path(__file__).resolve().parent
CONFIG_TOML     = _SCRIPT_DIR / "config.toml"
...
_CONFIG_TEMPLATE = "\n".join([
    "# histology_qc_viewer  --  configuration file",
    "# Generated by: python histology_qc_viewer.py --init-config",
    "# Edit the values below, then launch: python histology_qc_viewer.py",
    "",
    "[paths]",
    "# Absolute path to your cohort root directory.",
    "# All other paths are derived from this unless overridden below.",
    'base_dir   = "~/Microscopy/MyCohort"',
    "",
    "# Subdirectory (relative to base_dir) containing per-rat PNG folders.",
    "# Layout: <png_subdir>/<ratID>/CH1/<stem>_CH1.png  etc.",
    'png_subdir  = "PNG_fullsize"',
    "",
    "# Subdirectory containing predicted_order_<ratID>.csv section-order files.",
    'sort_subdir = "features/sorting"',
    "",
    "[channels]",
    "# Keys must be CH1..CH4.  Values are human-readable labels shown in the UI.",
    'CH1 = "DAPI"',
    'CH2 = "GFP"',
    'CH3 = "RFP"',
    'CH4 = "Cy5"',
    "",
    "[display]",
    "# Napari colourmap names per channel.",
    'CH1_colormap = "blue"',
    'CH2_colormap = "green"',
    'CH3_colormap = "red"',
    'CH4_colormap = "magenta"',
    "# Marker dot diameter in screen pixels.",
    "point_size = 18",
    "",
    "[behaviour]",
    "# Set to true to load only manifest-included sections by default.",
    "# Equivalent to always passing --manifest-only on the CLI.",
    "manifest_only = false",
    "",
])
...
def _load_config(toml_path: Path) -> None:
    """Read config.toml and update module-level globals in place."""
    global BASE_DIR, PNG_SUBDIR, PNG_DIR, SORT_DIR, DATASET_DIR
    global QC_CSV, COUNTS_CSV, MANIFEST_CSV, MANIFEST_ONLY
    global CHANNELS, CH_LABELS, CH_COLORMAPS, POINT_SIZE

    if tomllib is None:
        print(
            "[WARN] config.toml found but neither tomllib (Python 3.11+) nor "
            "tomli is available.\n"
    ...
    if "png_subdir" in paths:
        PNG_SUBDIR = paths["png_subdir"]
    sort_sub = paths.get("sort_subdir", "features/sorting")

    PNG_DIR      = BASE_DIR / PNG_SUBDIR
    SORT_DIR     = BASE_DIR / sort_sub
    DATASET_DIR  = BASE_DIR / "dataset"
    QC_CSV       = BASE_DIR / "qc_annotations.csv"
    COUNTS_CSV   = BASE_DIR / "cell_counts.csv"
    MANIFEST_CSV = BASE_DIR / "section_manifest.csv"
    ...
    for k in list(CHANNELS):
        if k in ch_cfg:
            CH_LABELS[k] = ch_cfg[k]

    disp = cfg.get("display", {})
    for k in list(CHANNELS):
        key = f"{k}_colormap"
        if key in disp:
            CH_COLORMAPS[k] = disp[key]
    if "point_size" in disp:
        POINT_SIZE = int(disp["point_size"])

    beh = cfg.get("behaviour", {})
    if "manifest_only" in beh:
        MANIFEST_ONLY = bool(beh["manifest_only"])

    print(f"[INFO] Config loaded from {toml_path}")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="histology_qc_viewer",
        description=(
            "Napari-based multichannel histology QC viewer with cell counting.\n"
            "Run --init-config to generate a starter config.toml."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p.add_argument(
        "--base", metavar="PATH", type=Path, default=None,
        help="Override base cohort directory from config.toml",
    )
    p.add_argument(
        "--png-subdir", metavar="DIR", default=None,
        help="PNG subdirectory name relative to --base (default: PNG_fullsize)",
    )
    p.add_argument(
        "--sort-subdir", metavar="DIR", default=None,
        help="Sorting CSV subdirectory relative to --base (default: features/sorting)",
    )
    p.add_argument(
        "--manifest-only", action="store_true",
        help="Load only sections marked include=1 in section_manifest.csv",
    )
    p.add_argument(
        "--config", metavar="PATH", type=Path, default=None,
        help="Path to a specific config.toml (default: config.toml next to this script)",
    )
    p.add_argument(
        "--init-config", action="store_true",
        help="Write a starter config.toml next to this script and exit",
    )
    return p
...
def main() -> None:
    ...
    # ── --init-config: write template and exit ────────────────────────────
    if args.init_config:
        dest = CONFIG_TOML
        if dest.exists():
            print(f"[INFO] config.toml already exists at {dest}")
            print("       Delete it first if you want to regenerate.")
        else:
            dest.write_text(_CONFIG_TEMPLATE)
            print(f"[INFO] config.toml written → {dest}")
            print("       Edit it to point at your dataset, then run:")
            print("         python histology_qc_viewer.py")
        sys.exit(0)

    # ── Load config.toml (no-op if missing) ───────────────────────────────
    toml_path = args.config if args.config else CONFIG_TOML
    ...
    if args.base:
        ...
        COUNTS_CSV   = BASE_DIR / "cell_counts.csv"
        MANIFEST_CSV = BASE_DIR / "section_manifest.csv"
        print(f"[INFO] --base override: {BASE_DIR}")

    if args.png_subdir:
        PNG_DIR = BASE_DIR / args.png_subdir

    if args.sort_subdir:
        SORT_DIR = BASE_DIR / args.sort_subdir

    if args.manifest_only:
        MANIFEST_ONLY = True
        print(f"[INFO] --manifest-only active: filtering to include=1 in {MANIFEST_CSV}")
